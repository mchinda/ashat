...
// async update(body:UsersEntity): Promise<UpdateResult> {
//   body.password = await this.getHash(body.password);
//   return await this.userRepository.update(body.id, body);
// }
...
async compareHash(password: string, hash: string): Promise<boolean> {
  ...
  /**@Method findUserAndRole()
  **Renvoie les utilisateurs les roles et la ligne_user_poste
  **/
  async findUserAndRole(){
      let [results, total] = await this.userRepository
                .createQueryBuilder("user")
                .leftJoinAndSelect("user.role","role")
                .getManyAndCount();//permet de compter le nombre de resultats retourner
      return new Pagination<UsersEntity>({results,total});
  ...
  /**@Method getPosteByUser()
  ** Retourne la liste des postes dont cette utilisateur a acces
  **@Params body: contient le login
  **/
  async getPosteByUser(body:any){
    let [results, total] = await this.userRepository
                  .createQueryBuilder("user")
                  .leftJoinAndSelect("user.role","role")
                  .where("user.login = :login", {login :body.login})
                  .getManyAndCount();
          return results;
    }
